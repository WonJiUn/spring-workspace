...
function replyData(){
	//데이터베이스에서 답글 데이터를 얻어옴
    $.ajax({
       url:"replyData/"+${personalData.writeNo}, type:"GET", //원글의 글번호를 넘겨줘서 답글들을 가져옴
       dataType:"json",
       success: function(rep){
          let html = ""
          rep.forEach(function(data){	//답글의 개수만큼 반복해서 추가됨
             let date = new Date(data.write_date)	//초단위로 된 시간데이터의 형식을 바꿔줌
	...
	function rep() {
		...
		//console.log(arr) //로그를 찍어보면 arr이 어떤식으로 만들어졌는지 알수있다->컨트롤러에서 map으로 받아줌
